...
class FormExtractor extends AbstractExtractor {
    ...
    public void process(final TemplateMetadata metadata, final Document document) {
        document.select(CONSTANT_FORM).groupBy( {
            ...
            def elementMetadata = new ElementMetadata(element.key, ElementType.FORM)
            elementMetadata.addAttribute(CONSTANT_ACTION, element.value.first().attr(CONSTANT_ACTION))

            metadata.addElementMetadata(elementMetadata);
        };
